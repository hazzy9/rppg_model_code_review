...
### Figure 1(A)
"""
Figure 1(A) - Agreement between reference and predicted FPG.
...
plt.tight_layout()
plt.savefig("figureA_agreement_plot.png", dpi=300)
plt.show()


### Figure 1(B) : R-programming

### Figure 1(C)
"""
Figure 1C - Calibration stability over time: MARD by days-since-
calibration, uncalibrated vs. decayed-calibration (half-life 30 days).

# reads:  figure1C_source_data.csv
One row per time bin (5 rows total) - bootstrap mean + 95% CI 
across subjects, not per-subject data. Has more columns than
this figure plots (MAE, RMSE, "Frozen" calibration, 14d/60d
half-life variants) - those are the sensitivity analysis behind
picking 30d as primary, left in rather than trimmed out.
...
for bin_name, color in palette.items():
    ...
    plt.scatter(
        sub["pc1"], sub["pc2"],
        s=40, c=color, alpha=alphas, edgecolors="none", label=bin_name,
    )
...
for ax, pc, label, r, p in [
    (axes[0], pc1, "PC1", r1, p1),
    (axes[1], pc2, "PC2", r2, p2),
]:
...
"""
Figure 2C - Frequency importance: |loading| averaged over channels and
time, for PC1 and PC2
...
def reconstruct_loadings(components_top2, scale_, support_mask, tfm_shape):
    """Undo variance-threshold + standardization to get (PC, C, F, T) loadings."""
    C, F, T = tfm_shape
...
L = reconstruct_loadings(components_top2, scale_, support_mask, (C, F, T))  # (2, C, F, T)
...
for ax, pc_idx, label in zip(axes, [0, 1], ["PC1", "PC2"]):
    ...
    im = ax.imshow(
        tf_crop, origin="lower", aspect="auto",
...
anova_stability (F, T) proportion of bootstrap draws where each pixel fell in a significant ANOVA cluster
f_axis, t_axis (F,), (T,)
...
"""
Figure 2F - Delta-power between clusters in three groups (CrCb, vs <100
reference).
# reads:  figure2efg_source_data.npz 
...
for ax, g, title in zip(axes, [0, 1, 2], titles):
    ...
    im = ax.imshow(
        masked_diff, aspect="auto", origin="lower",
        extent=[t_axis[0], t_axis[-1], f_axis[0], f_axis[-1]],
...
"""
Figure 2G - Quadratic fit: cluster power vs FPG

# reads:  figure2efg_source_data.npz
fpg, neg_log_likelihood
...
#=== Figure 3
"""
Figure 3A - average time-frequency maps
...
for bin_name in BIN_ORDER:
    ...
    if len(idx) == 0:
        continue
    avg_specs[bin_name] = images[idx, :, :, c_idx].mean(axis=0)   # (H, W)
...
"""
Figures 3B-D - Cohen's d effect-size maps (POS, CrCb, PBV), aligned
...
# label used inside the "(bin_b - bin_a)" title text
